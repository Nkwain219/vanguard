...
exports.sendPushNotification = exports.recalculateWalletSummary = exports.disburseSalary = exports.createUser = exports.checkBalance = exports.collectPayment = void 0;
...
const mesomb_1 = require("@hachther/mesomb");
// Initialize Firebase Admin
admin.initializeApp();
const db = (0, firestore_2.getFirestore)("vanguard-db");
// Define secrets using the modern params API
// MeSomb requires: applicationKey, accessKey, and secretKey
const mesombAppKey = (0, params_1.defineSecret)("MESOMB_APP_KEY");
const mesombAccessKey = (0, params_1.defineSecret)("MESOMB_ACCESS_KEY");
const mesombSecretKey = (0, params_1.defineSecret)("MESOMB_SECRET_KEY");
const smtpHost = (0, params_1.defineSecret)("SMTP_HOST");
const smtpPort = (0, params_1.defineSecret)("SMTP_PORT");
const smtpSecure = (0, params_1.defineSecret)("SMTP_SECURE");
 ...
 */
exports.collectPayment = (0, https_1.onCall)({ secrets: [mesombAppKey, mesombAccessKey] }, async (request) => {
    // Ensure user is authenticated
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "You must be logged in to make a payment.");
    }
    ...
    if (!validServices.includes(normalizedService)) {
        throw new https_1.HttpsError("invalid-argument", "Service must be MTN or ORANGE.");
    }
    const appKey = mesombAppKey.value();
    const accessKey = mesombAccessKey.value();
    if (!appKey || !accessKey) {
        throw new https_1.HttpsError("failed-precondition", "Payment service is not configured. Please contact support.");
    }
    const reference = `txn_${Date.now()}_${userId.substring(0, 8)}`;
    try {
        // Call MeSomb API to collect payment
        const response = await axios_1.default.post(`${MESOMB_BASE_URL}/payment/collect/`, {
        ...
        if (axios_1.default.isAxiosError(error)) {
            ...
            throw new https_1.HttpsError("aborted", errorMessage);
        }
 ...
 * SDK: @hachther/mesomb
 */
exports.checkBalance = (0, https_1.onCall)({ secrets: [mesombAppKey, mesombAccessKey, mesombSecretKey] }, async (request) => {
    if (!request.auth) {
    ...
    if (!appKey || !accessKey || !secretKey) {
        ...
        throw new https_1.HttpsError("failed-precondition", "MeSomb credentials not configured. Please add MESOMB_SECRET_KEY.");
    }
    try {
        // Use official MeSomb SDK for balance
        const payment = new mesomb_1.PaymentOperation({
        ...
        let totalDebit = 0;
        try {
            const statsResponse = await axios_1.default.get(`${MESOMB_BASE_URL}/application/status/`, {
                headers: {
                    "Content-Type": "application/json",
                    "X-MeSomb-Application": appKey,
                    "Authorization": `Token ${accessKey}`,
        ...
        throw new https_1.HttpsError("internal", `Could not retrieve balance: ${errorMessage}`);
    }
});
/**
 ...
 */
exports.createUser = (0, https_1.onCall)({ secrets: [smtpHost, smtpPort, smtpSecure, smtpUser, smtpPass, smtpFrom] }, async (request) => {
    ...
    if (!request.auth) {
        ...
        throw new https_1.HttpsError("unauthenticated", "You must be logged in to create users.");
    }
    // Verify caller is admin
    const callerDoc = await db.collection("vanguard_users").doc(request.auth.uid).get();
    const callerData = callerDoc.data();
    if (callerData?.role !== "admin") {
        throw new https_1.HttpsError("permission-denied", "Only admins can create new users.");
    }
    const data = request.data;
    const { email, firstName, lastName, phone, role, department, position, salary, bankAccount, idCard } = data;
    // Validate required fields
    if (!email || !firstName || !lastName || !role) {
        throw new https_1.HttpsError("invalid-argument", "Email, first name, last name, and role are required.");
    }
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", "Failed to create user. Please try again.");
    }
});
/**
 * Generate a secure random password.
 ...
 */
exports.disburseSalary = (0, https_1.onCall)({ secrets: [mesombAppKey, mesombAccessKey] }, async (request) => {
    // Ensure user is authenticated
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "You must be logged in to disburse salaries.");
    }
    // Verify caller is admin
    const callerDoc = await db.collection("vanguard_users").doc(request.auth.uid).get();
    const callerData = callerDoc.data();
    if (callerData?.role !== "admin") {
        throw new https_1.HttpsError("permission-denied", "Only admins can disburse salaries.");
    }
    const data = request.data;
    ...
    if (!requestDoc.exists) {
        throw new https_1.HttpsError("not-found", "Salary request not found.");
    }
    const requestData = requestDoc.data();
    // Check if already processed
    if (requestData.status !== "pending") {
        throw new https_1.HttpsError("failed-precondition", `This request has already been ${requestData.status}.`);
    }
    ...
    if (!phoneNumber || phoneNumber.length < 9) {
        throw new https_1.HttpsError("failed-precondition", "Employee does not have a valid phone number.");
    }
    ...
    if (walletBalance < amount) {
        throw new https_1.HttpsError("failed-precondition", `Insufficient wallet balance. Available: ${walletBalance} XAF, Required: ${amount} XAF`);
    }
    ...
    if (!mobileService) {
        ...
        if (prefix === "67" || mtnPrefixes.includes(prefix3)) {
            ...
            throw new https_1.HttpsError("invalid-argument", "Could not detect mobile money service. Please specify MTN or ORANGE.");
        }
    }
    const appKey = mesombAppKey.value();
    const accessKey = mesombAccessKey.value();
    ...
    try {
        ...
        const response = await axios_1.default.post(`${MESOMB_BASE_URL}/payment/deposit/`, {
            amount: Math.round(amount),
        ...
        throw new https_1.HttpsError("internal", "An unexpected error occurred during payment. Please try again later.");
    }
});
/**
 ...
 */
exports.recalculateWalletSummary = (0, https_1.onCall)(async (request) => {
    if (!request.auth) {
        throw new https_1.HttpsError("unauthenticated", "Must be logged in.");
    }
    // Get user's role from Firestore to verify admin
    ...
    try {
        ...
        const errorMessage = error instanceof Error ? error.message : "Unknown error";
        throw new https_1.HttpsError("internal", `Failed to recalculate: ${errorMessage}`);
    }
});
/**
 ...
 */
exports.sendPushNotification = (0, firestore_1.onDocumentCreated)({ document: "vanguard_notifications/{notificationId}" }, async (event) => {
